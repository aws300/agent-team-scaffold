...
这套循环已经收成一个**领域无关、可直接 `git clone` 的脚手架** `agent-team-scaffold`。它和第 9 章那套轻量 CMA 蓝图（`build.py` + `cma.yaml` 从 md 派生、experts/workflows 两轴、零 sync）外层结构、编译器**逐字相同**，区别只在：它的 experts 不按领域职能组织，而**按循环角色**组织。
...
- **可复制骨架**：`agent-team-scaffold` 把这一切收成一份 `git clone` 即用的脚手架，外层与 CMA 编译器和第 9 章轻量蓝图逐字相同，只是 experts 按循环角色组织。
